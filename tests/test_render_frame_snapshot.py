...
def test_build_render_frame_contains_cards_and_charts() -> None:
    ...
    config.viruses.initial_count = 1
    ...
    env.reset(seed=7)
    env.world.eject_mass(env.agent_ids[0], np.array([1.0, 0.0], dtype=np.float32))
    ...
    frame = build_render_frame(
        ...
        focus_agent_index=1,
        last_actions={env.agent_ids[0]: np.array([1.0, 0.0, 1.0], dtype=np.float32)},
    )

    assert frame.world.focus_agent_id == "agent_1"
    assert len(frame.world.viruses) == 1
    assert len(frame.world.ejected_masses) >= 0
    assert frame.agent_intents[0].agent_id == env.agent_ids[0]
    assert frame.scenario.name
    assert frame.reward_breakdowns
